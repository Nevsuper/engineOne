...
Texture2D::Texture2D(int width, int height, const void* data, pixelDataType type, TextureInternalFormat internalFormat, int mipLevels) noexcept
{
	...
	glTextureStorage2D(m_ID, mipLevels, TextureInternalFormatToGLenum(internalFormat), width, height);
	TextureBaseFormat baseFormat = GetBaseFormatFromInternalFormat(internalFormat);

	SubImage(width, height, data, type, baseFormat);
}
